...
async def handle_request(request: Request):
    ...
    session_id =help.extract_session_id(output_contexts[0]["name"])
    intent_handler_dict = {
...
def add_order(parameters:dict,session_id:str):
    ...
    if len(food_items) != len(quantities):
        ...
        order_str = help.get_str_from_food_dict(inprogress_orders[session_id])
...
def remove_from_order(parameters: dict, session_id: str):
    ...
    if len(current_order.keys()) == 0:
        ...
        order_str = help.get_str_from_food_dict(current_order)
...
def track_order(parameters:dict,session_id:str):
    ...
    if order_id is None:
        return {"error": "order_id does not exist"}
    order_status=dp_helper.get_order_status(order_id)
    if order_status:
        fulfillment_text=f"the oder status for order id :{order_id} is:{order_status}"
    else:
